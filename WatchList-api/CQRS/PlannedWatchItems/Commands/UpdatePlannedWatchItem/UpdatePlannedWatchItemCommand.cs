...
namespace WatchList_api.CQRS.PlannedWatchItems.Commands.CreatePlannedWatchItem
{
    public class UpdatePlannedWatchItemCommand : ICommand<UpdatePlannedWatchItemRequest, UpdatePlannedWatchItemResponse>, IAutoRegisterCommand
    {
        ...
        public async Task<UpdatePlannedWatchItemResponse> ExecuteAsync(UpdatePlannedWatchItemRequest request)
        {
            using (var conn = _connection.GetConnection())
            {
                ...
                var result = await conn.ExecuteAsync(sql, new { Id = request.Id, UserId = request.UserId, Priority = request.WatchItem.Priority });
